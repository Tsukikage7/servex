...
func (m *redisQuotaManager) Reset(ctx context.Context, key string) error {
	...
	for iter.Next(ctx) {
		if err := m.client.Del(ctx, iter.Val()).Err(); err != nil {
			slog.ErrorContext(ctx, "删除配额键失败", "component", "RateLimit", "key", iter.Val(), "error", err)
		}
	}
	if err := iter.Err(); err != nil {
		slog.ErrorContext(ctx, "SCAN 迭代配额键失败", "component", "RateLimit", "pattern", pattern, "error", err)
		return err
	}
	return nil
}
