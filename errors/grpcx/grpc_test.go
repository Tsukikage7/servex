...
func TestToGRPCStatus(t *testing.T) {
	errTokenExpired := serrors.NewWithKind(100401, "auth.token.expired", "令牌已过期", serrors.KindUnauthenticated)

	t.Run("from *serrors.Error", func(t *testing.T) {
		st := ToGRPCStatus(errTokenExpired)
		assert.Equal(t, codes.Unauthenticated, st.Code())
		assert.Contains(t, st.Message(), "令牌已过期")
	})

	t.Run("from *serrors.Error without GRPC set", func(t *testing.T) {
		err := serrors.New(999, "unknown", "未知错误")
		st := ToGRPCStatus(err)
...
func TestToGRPCStatus_ErrorInfo(t *testing.T) {
	err := serrors.New(100401, "auth.token.expired", "令牌已过期").
		WithKind(serrors.KindUnauthenticated).
...
func TestToGRPCStatus_BadRequest(t *testing.T) {
	err := serrors.New(100400, "validation.failed", "参数校验失败").
		WithKind(serrors.KindInvalidArgument).
...
func TestToGRPCStatus_RetryInfo(t *testing.T) {
	err := serrors.New(100429, "rate.limited", "请求过于频繁").
		WithKind(serrors.KindResourceExhausted).
...
func TestFromGRPCStatus(t *testing.T) {
	t.Run("round trip", func(t *testing.T) {
		original := serrors.New(100401, "auth.token.expired", "令牌已过期").
			WithKind(serrors.KindUnauthenticated)
		...
		assert.Equal(t, serrors.KindUnauthenticated, restored.Kind)
	})
...
func TestFromGRPCStatus_WithBadRequest(t *testing.T) {
	original := serrors.New(100400, "validation.failed", "参数校验失败").
		WithKind(serrors.KindInvalidArgument).
...
func TestFromGRPCStatus_WithRetryInfo(t *testing.T) {
	original := serrors.New(100429, "rate.limited", "请求过于频繁").
		WithKind(serrors.KindResourceExhausted).
...
func TestUnaryServerInterceptor(t *testing.T) {
	...
	t.Run("handler returns *serrors.Error", func(t *testing.T) {
