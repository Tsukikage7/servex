...
`github.com/Tsukikage7/servex/v2/llm/router` 提供 Go 内部多 Provider 路由器，实现 `llm.ChatModel` 接口，根据调用时的 `llm.WithModel()` 选项将请求转发到对应的 Provider 客户端。

这个包只处理 Go 进程内的模型路由和 fallback；HTTP 入口、API Key、计费、审计和 SSE 输出由 `llm/gateway` 负责。
