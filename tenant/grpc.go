...
func UnaryServerInterceptor(resolver Resolver, opts ...Option) grpc.UnaryServerInterceptor {
	...
	return func(
		...
		if err != nil {
			if o.logger != nil {
				logger.FromContext(ctx).Debug("[Tenant] gRPC令牌提取失败",
		...
		if err != nil {
			if o.logger != nil {
				logger.FromContext(ctx).Warn("[Tenant] gRPC解析失败",
					logger.String("method", info.FullMethod),
					logger.Err(err),
				)
			}
		...
		if !t.TenantEnabled() {
			if o.logger != nil {
				logger.FromContext(ctx).Warn("[Tenant] gRPC租户已禁用",
...
func StreamServerInterceptor(resolver Resolver, opts ...Option) grpc.StreamServerInterceptor {
	...
	return func(
		...
		if err != nil {
			if o.logger != nil {
				logger.FromContext(ctx).Debug("[Tenant] gRPC流令牌提取失败",
					logger.String("method", info.FullMethod),
					logger.Err(err),
				)
			}
		...
		if err != nil {
			if o.logger != nil {
				logger.FromContext(ctx).Warn("[Tenant] gRPC流解析失败",
					logger.String("method", info.FullMethod),
					logger.Err(err),
				)
			}
		...
		if !t.TenantEnabled() {
			if o.logger != nil {
				logger.FromContext(ctx).Warn("[Tenant] gRPC流租户已禁用",
					logger.String("tenant_id", t.TenantID()),
					logger.String("method", info.FullMethod),
				)
			}
