...
`transport/response` 内置中英文错误消息，会从 `Accept-Language` 选择语言。业务应用可以在启动时替换全局消息包，错误定义只保留稳定的 message key。
...
规则：
- `Accept-Language` 可直接传给 `response.LocalizedMessage` 或由 `WriteLocalizedError` 自动读取。
- 非内部错误的显式自定义消息优先于翻译。
- 内部错误会返回通用本地化消息，避免暴露底层细节。
