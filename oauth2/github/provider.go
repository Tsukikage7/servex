...
// AuthURL 生成 GitHub OAuth2 授权跳转链接含 PKCE.
...
func (p *Provider) ExchangeWithState(ctx context.Context, code, state string) (*oauth2.Token, error) {
	...
	if errMsg, ok := result["error"]; ok {
		return nil, oauth2.ErrExchangeFailed.WithMessage(fmt.Sprintf("code 换取 token 失败: %v", errMsg))
	}
...
// Refresh 刷新访问令牌GitHub 不支持 refresh token.
