...
func runNew(args []string) error {
	...
	module := fs.String("module", "", "Go module path (default: github.com/example/<project>)")
	standalone := fs.Bool("standalone", false, "Create standalone single-service project instead of monorepo")
	withGRPC := fs.Bool("with-grpc", false, "Include gRPC server setup")
	withDB := fs.Bool("with-db", false, "Include database (GORM) setup")
	withRedis := fs.Bool("with-redis", false, "Include Redis setup")
	withWire := fs.Bool("with-wire", false, "Include Wire dependency injection setup")
	fs.Usage = func() {
		fmt.Println("Usage: servex new <project-name> [options]")
		fmt.Println()
		fmt.Println("Options:")
		fs.PrintDefaults()
	}
...
func generateProject(data ProjectData) error {
	...
	for _, tf := range templateFiles {
		if err := renderTemplate(projectTemplates, tf.tmpl, filepath.Join(data.Name, tf.out), data, funcMap); err != nil {
			return fmt.Errorf("render %s: %w", tf.tmpl, err)
		}
	}
	...
	fmt.Printf("Project %q created successfully! (standalone)\n", data.Name)
...
func generateMonorepo(data ProjectData) error {
	...
	fmt.Printf("Project %q created successfully! (monorepo)\n", data.Name)
	fmt.Printf("  cd %s && go mod tidy\n", data.Name)
	fmt.Println("  servex add service <name> --with-grpc --with-db")
	return nil
}
