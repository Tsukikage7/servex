	...
	if !exists {
		...
		if counter, exists = c.counters[name]; !exists {
			...
			if err := c.registry.Register(counter); err == nil {
				...
				slog.Error("注册自定义 Counter 失败", slog.String("component", "Metrics"), slog.String("name", name), slog.Any("error", err))
			}
		}
		c.mu.Unlock()
	}
	...
	if !exists {
		...
		if histogram, exists = c.histograms[name]; !exists {
			...
			if err := c.registry.Register(histogram); err == nil {
				...
				slog.Error("注册自定义 Histogram 失败", slog.String("component", "Metrics"), slog.String("name", name), slog.Any("error", err))
			}
		}
		c.mu.Unlock()
	}
	...
	if !exists {
		...
		if gauge, exists = c.gauges[name]; !exists {
			...
			if err := c.registry.Register(gauge); err == nil {
				...
				slog.Error("注册自定义 Gauge 失败", slog.String("component", "Metrics"), slog.String("name", name), slog.Any("error", err))
			}
		}
		c.mu.Unlock()
	}
