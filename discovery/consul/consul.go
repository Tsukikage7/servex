...
func init() {
	discovery.MustRegister(discovery.TypeConsul, New)
}
...
// New 创建 Consul 服务发现实例.
func New(config *discovery.Config, log logger.Logger) (discovery.Discovery, error) {
	...
	if err != nil {
		...
		return nil, discovery.ErrClientCreate
	}
...
func (c *consulDiscovery) Register(ctx context.Context, serviceName, address string) (string, error) {
	if serviceName == "" {
		return "", discovery.ErrEmptyName
	}

	if address == "" {
		return "", discovery.ErrEmptyAddress
	}
	...
	// 生成唯一的服务ID
	serviceID := discovery.GenerateServiceID(serviceName)

	// 创建健康检查配置
	healthCheck := c.buildHealthCheck(serviceID, serviceName, host, port)

	// 使用gRPC作为默认配置
	defaultMeta := c.config.GetServiceConfig(discovery.ProtocolGRPC)
...
func (c *consulDiscovery) RegisterWithProtocol(ctx context.Context, serviceName, address, protocol string) (string, error) {
	...
	// 生成协议特定的服务ID
	serviceID := discovery.GenerateServiceID(fmt.Sprintf("%s-%s", serviceName, protocol))
...
func (c *consulDiscovery) Unregister(ctx context.Context, serviceID string) error {
	...
	case err := <-errChan:
		if err != nil {
			...
			return discovery.ErrUnregister
		}
	}
...
func (c *consulDiscovery) Discover(ctx context.Context, serviceName string) ([]string, error) {
	...
	if err != nil {
		...
		return nil, discovery.ErrDiscover
	}
...
func parseAddress(address string) (string, int, error) {
	...
	if err != nil {
		return "", 0, discovery.ErrInvalidAddress
	}

	port, err := strconv.Atoi(portStr)
	if err != nil {
		return "", 0, discovery.ErrInvalidPort
	}
